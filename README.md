...
9.5 Детализация процедур тестирования

Этот подраздел предоставляет подробные инструкции для запуска тестов, что упрощает процесс тестирования и валидации для разработчиков.

	•	Инструкции по запуску тестов: Для выполнения тестов необходимо перейти в корневую директорию проекта и запустить команду pytest, которая инициирует выполнение всех тестов, находящихся в папке tests/.
Пример команды для запуска:

pytest tests/

В случае успешного прохождения тестов будет показано соответствующее уведомление. Если какие-либо тесты не прошли, pytest выведет подробную информацию об ошибках, что упростит их диагностику.

	•	Документация по ошибкам тестирования: Рекомендуется при возникновении ошибок сохранять логи в отдельный файл для анализа и последующей отладки. Для этого можно использовать параметр --result-log:

pytest --result-log=tests/logs/test_results.log

Этот лог содержит информацию о каждом выполненном тесте и его статусе, что помогает в случае массовых ошибок.
