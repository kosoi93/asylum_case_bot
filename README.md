...
- **Удобный интерфейс**: Взаимодействие с пользователями через кнопки и автоматизированные уведомления.
- **Пользовательское соглашение**: Обязательное согласие перед загрузкой файлов.
- **Загрузка PDF-файлов**: Поддержка загрузки политических кейсов в формате PDF.
- **Анализ данных**: Обработка загруженного текста с использованием OpenAI API для выявления несоответствий и формирования рекомендаций.
- **Создание отчетов**: Генерация PDF-отчетов на основе результатов анализа.
- **Обработка ошибок и уведомления**: Уведомления об ошибках и возможность связи с поддержкой.
- **Логирование**: Запись всех событий и ошибок для удобства анализа и диагностики.
